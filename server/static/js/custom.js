...
$(window).load(function() {
    ...
    // Force HTTPS redirect if not being used
...
$(document).ready(function() {
  ...
  $(".mailchimp-subscribe").ajaxChimp({
      ...
      url: "https://launchalarm.us13.list-manage.com/subscribe/post?u=1d0cb249d0d7310f727261783&amp;id=18fb5b0ed5" // Replace your mailchimp post url inside double quote "".
  });
  ...
  function mailchimpCallback(resp) {
    ...
    if(resp.result === 'success') {
        $('.mc-success')
        .html('<i class="icon icon_check_alt2"></i> <strong>'+resp.msg.substring(4)+'</strong>')
        .fadeIn(1000);
        ...
        $('.mc-failed')
        .html('<i class="icon icon_close_alt2"></i> <strong>'+resp.msg.substring(4)+'</strong>')
        .fadeIn(1000);
  ...
  if(parameters[0]){ document.getElementById('confirm-email').value = parameters[0]; };
  if(parameters[1]){ document.getElementById('confirm-code').value = parameters[1]; };

  $("#confirm-divider").submit(function(e) {
      e.preventDefault();
      var data = {
      ...
      if ( isValidEmail(data['email']) && (data['confirmCode'].length > 1)) {
          $.ajax({
              type: "POST",
              url: "v1/account/signup/confirm",
              headers: {
              ...
              success: function(response) {
                  $('.confirm-success').html('<i class="icon icon_check_alt2"></i> <strong> '+response.email+' - '+response.msg+'</strong>');
                  $('.confirm-success').fadeIn(1000);
                  $('.confirm-failed').fadeOut(0);
              },
              error: function(response) {
                  var json_response = $.parseJSON(response.responseText);
                  $('.confirm-failed').html('<i class="icon icon_close_alt2"></i><strong> '+json_response.err+' - '+json_response.msg+'</strong>');
                  $('.confirm-failed').fadeIn(1000);
                  $('.confirm-success').fadeOut(500);
              }
          });
      } else {
          var json_response = $.parseJSON(response.responseText);
      ...
      // success: function(response) {
      //   var json_response = $.parseJSON(response.responseText);
      //   $("#ajaxResponse").append(json_response.email+" - "+json_response.msg);
      // },
      // //if received a response from the server
      // error: function(response) {
      //   var json_response = $.parseJSON(response.responseText);
      //   $("#ajaxResponse").append(json_response.err+" - "+json_response.msg);
      // },
  ...
  /* =======================================================================
     DIVIDER ANIMATED POLYGON BACKGROUND
  ========================================================================== */
  var container = document.getElementById('confirm');
